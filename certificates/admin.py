...
class CertificateAdmin(admin.ModelAdmin):
    ...
    """
    list_display = (
    'name', 'supplier', 'issue_date', 'expiry_date', 'approved', 'get_qr_code_image', 'suspected_tampered',
    'upload_time')
    list_filter = ('approved', 'suspected_tampered', 'upload_time', 'expiry_date')
    search_fields = ('name', 'supplier__email', 'supplier__first_name', 'supplier__last_name')
    readonly_fields = (
    'get_qr_code_image', 'file_hash', 'upload_time', 'version', 'previous_versions', 'last_checked', 'certificate_qr')

    fieldsets = (
        (None, {
            'fields': ('supplier', 'name', 'description', 'file', 'get_qr_code_image')
        }),
        (_('Dates & Versions'), {
            'fields': ('issue_date', 'expiry_date', 'upload_time', 'last_checked', 'version', 'previous_versions')
        }),
        (_('Verification & Integrity'), {
            'fields': ('file_hash', 'verified', 'suspected_tampered', 'approval_status', 'approved', 'certificate_qr')
        }),
    )
    ...
    def get_qr_code_image(self, obj):
        ...
        if obj.certificate_qr and obj.certificate_qr.qr_code_image:
            return format_html(f'<img src="{obj.certificate_qr.qr_code_image.url}" width="100" height="100" />')
        return _("No QR Code")
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.approval_status == ApprovalStatus.APPROVED and not obj.certificate_qr:
            certificate_qr = CertificateQR.objects.create(supplier=obj.supplier, certificate_id=obj.pk)
