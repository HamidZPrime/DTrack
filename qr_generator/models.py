...
class SupplierQR(models.Model):
    ...
    """
    supplier = models.OneToOneField(CustomUser, on_delete=models.CASCADE, limit_choices_to={'role': 'supplier'}, verbose_name=_("Supplier"))
    qr_code_image = models.ImageField(_("QR Code Image"), upload_to='qr_codes/suppliers/', blank=True)
    qr_token = models.UUIDField(_("QR Token"), default=uuid.uuid4, editable=False, unique=True)
...
class ProductQR(models.Model):
    ...
    """
    supplier = models.ForeignKey(CustomUser, on_delete=models.CASCADE, limit_choices_to={'role': 'supplier'}, verbose_name=_("Supplier"))
    product_id = models.CharField(_("Product ID"), max_length=100)
    qr_code_image = models.ImageField(_("QR Code Image"), upload_to='qr_codes/products/', blank=True)
    qr_token = models.UUIDField(_("QR Token"), default=uuid.uuid4, editable=False, unique=True)
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
...
class CertificateQR(models.Model):
    ...
    qr_code_image = models.ImageField(_("QR Code Image"), upload_to='qr_codes/certificates/', blank=True)
    qr_token = models.UUIDField(_("QR Token"), default=uuid.uuid4, editable=False, unique=True)
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
