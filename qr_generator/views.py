...
def supplier_detail(request, qr_token):
    ...
    return render(request, 'qr_generator/supplier_detail.html', {'supplier': supplier_qr.supplier})

def product_detail(request, qr_token):
    product_qr = get_object_or_404(ProductQR, qr_token=qr_token)
    return render(request, 'qr_generator/product_detail.html', {'product': product_qr})

def certificate_detail(request, qr_token):
    certificate_qr = get_object_or_404(CertificateQR, qr_token=qr_token)
    return render(request, 'qr_generator/certificate_detail.html', {'certificate': certificate_qr})
