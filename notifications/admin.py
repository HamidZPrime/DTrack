...
from .models import NotificationMethod, NotificationRule, NotificationHistory, UserNotificationPreferences, Notification, NotificationSchedule
...
class NotificationRuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'trigger', 'condition', 'notification_methods', 'is_active', 'priority')
        }),
        ('Templates', {
...
class UserNotificationPreferencesAdmin(admin.ModelAdmin):
    list_display = ('user', 'email_notifications', 'sms_notifications', 'voice_notifications', 'preferred_language')
    search_fields = ('user__email',)
    list_filter = ('email_notifications', 'sms_notifications', 'voice_notifications', 'preferred_language')
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('user', 'rule', 'method', 'status', 'message_id', 'additional_data', 'sent_at', 'created_at')
        }),
    )
...
class NotificationScheduleAdmin(admin.ModelAdmin):
    list_display = ('rule', 'schedule_time', 'is_recurring', 'recurrence_interval', 'last_sent_at')
    search_fields = ('rule__name',)
    list_filter = ('is_recurring',)
    fieldsets = (
        (None, {
            'fields': ('rule', 'schedule_time', 'is_recurring', 'recurrence_interval', 'last_sent_at')
        }),
    )
