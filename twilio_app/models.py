...
class TwilioMessageLog(models.Model):
    ...
    message_type = models.CharField(
        _("Message Type"), max_length=10, choices=MessageType.choices
    )
    ...
    def __str__(self):
        return (
            f"{self.message_type.title()} Message to {self.user}"
            if self.user
...
class TwilioService:
    ...
    def validate_twilio_settings():
        ...
        if missing_settings:
            raise ValueError(
                f"Missing required setting(s): {', '.join(missing_settings)}"
            )
    ...
    def make_voice_call(to, url):
        ...
        try:
            call = client.calls.create(
                to=to, from_=settings.TWILIO_PHONE_NUMBER, url=url
            )
