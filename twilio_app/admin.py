...
class TwilioMessageLogAdmin(admin.ModelAdmin):
    list_display = ('user', 'message_type', 'status', 'message_sid', 'sent_at', 'created_at')
    search_fields = ('user__email', 'message_sid', 'status', 'message_type')
    list_filter = ('status', 'message_type', 'sent_at')
    readonly_fields = ('user', 'message_type', 'message_sid', 'status', 'error_message', 'sent_at', 'created_at')
    fieldsets = (
        (None, {
            'fields': ('user', 'message_type', 'message_sid', 'status', 'error_message', 'sent_at', 'created_at')
        }),
    )
